...
var listaCompras = {
  ...
  while (preguntaInical === "si") {
    ...
    let eliminar = prompt("¿Quieres eliminar un producto? si/no").trim().toLowerCase();
    if (eliminar === "si") {
      let productoEliminar = prompt("¿Qué producto quieres eliminar?").trim().toLowerCase(); // Aquí preguntas por el producto
      let encontrado = false;
    
      for (let categoria in listaCompras) {
        if (listaCompras[categoria].includes(productoEliminar)) {
          let index = listaCompras[categoria].indexOf(productoEliminar);
          listaCompras[categoria].splice(index, 1);
          alert(`${productoEliminar} eliminado de la categoría ${categoria}`);
          encontrado = true;
      ...
      alert("No se eliminó ningún producto.");
    }
